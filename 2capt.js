...
const g2captcha = require('2captcha');
...
// ==========================================
const TWOCAPTCHA_API_KEY = 'TARUH_API_KEY_2CAPTCHA_DISINI'; 
const SITEKEY_CLOUDFLARE = '0x4AAAAAAAypg1U6LE_0hR1G'; // Sitekey Turnstile billsonchain.io

const solver = new g2captcha.Solver(TWOCAPTCHA_API_KEY);
...
// ====================================================================
// 🛠️ PERUBAHAN UTAMA: LOGIN DENGAN API + BYPASS 2CAPTCHA (NO BROWSER)
// ====================================================================
async function pemicuLoginOtomatisAkun(akun) {
    console.log(`   [2CAPTCHA] Mengirim tantangan Turnstile ke server solver...`);
    try {
        // 1. Selesaikan captcha via 2captcha API secara asinkronus
        const result = await solver.cloudflareTurnstile({
            pageurl: 'https://billsonchain.io/login',
            sitekey: SITEKEY_CLOUDFLARE
        });
        
        const tokenCaptcha = result.data;
        console.log(`   ✅ [2CAPTCHA] Token captcha berhasil didapatkan.`);

        console.log(`   [LOGIN ENGINE] Menembak payload request langsung ke API auth...`);
        
        // 2. Kirim request POST login langsung ke server endpoint auth
        const loginResponse = await axios.post('https://bocapi.billsonchain.io/api/auth/login', {
            email: akun.email,
            password: akun.password,
            captchaToken: tokenCaptcha // Menyisipkan token bypass
        }, {
            headers: {
                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36',
        ...
        // 3. Ekstrak cookie set-cookie header dari respon server
        const setCookieHeaders = loginResponse.headers['set-cookie'];
        if (!setCookieHeaders || setCookieHeaders.length === 0) {
            throw new Error('Gagal mendapatkan session token cookie dari response header.');
        }

        // Gabungkan string cookie array menjadi format satu baris raw
        const cookieString = setCookieHeaders.map(c => c.split(';')[0]).join('; ');
        console.log(`   ✅ [LOGIN ENGINE] Login via API Sukses tanpa browser.`);
        ...
        console.error(`   ❌ [LOGIN ENGINE API ERROR]: ${e.message}`);
        if (e.response && e.response.data) {
            console.error(`   Detail Refusal Server:`, JSON.stringify(e.response.data));
        }
        return null;
    }
}
...
// ====================================================================
// 🛠️ GENERATOR NAMA MERCHANT & NMID ACAK
// ====================================================================
...
function generateData() {
    ...
    const d = new Date(); d.setDate(d.getDate() - ri(0, 5)); d.setHours(ri(7, 22), ri(0, 59), ri(0, 59));
...
function drawQR(ctx, cx, cy, size) {
    const M     = 25; const cell  = size / M; ctx.fillStyle = '#ffffff'; ctx.fillRect(cx - size/2, cy - size/2, size, size);
...
function drawReceipt(data) {
    const SCALE = 3; const BASE_W = 420, BASE_H = 880; const W = BASE_W * SCALE, H = BASE_H * SCALE;
    const canvas = createCanvas(W, H); const ctx = canvas.getContext('2d'); ctx.scale(SCALE, SCALE);
    ...
    const grad = ctx.createLinearGradient(0, 0, 0, BASE_H); grad.addColorStop(0, '#dce3eb'); grad.addColorStop(1, '#ecf0f5'); ctx.fillStyle = grad; ctx.fillRect(0, 0, BASE_W, BASE_H);

    ctx.save(); ctx.shadowColor = 'rgba(16, 36, 54, 0.12)'; ctx.shadowBlur = 20; ctx.shadowOffsetY = 8; ctx.fillStyle = WHITE; roundRect(ctx, RX, RY, RW, RH, 14); ctx.fill(); ctx.restore();
    ctx.save(); ctx.fillStyle = BLUE; roundRect(ctx, RX, RY, RW, 72, 14); ctx.fill(); ctx.fillRect(RX, RY + 55, RW, 17); ctx.restore();
    ctx.fillStyle = WHITE; ctx.font = 'bold 22px Arial, Helvetica, sans-serif'; ctx.fillText('DANA', RX + 24, RY + 44); ctx.font = '12px Arial'; ctx.textAlign = 'right'; ctx.fillText('0821 •••• 6061', RX + RW - 24, RY + 44);

    const ICO_Y = RY + 105; ctx.save(); ctx.fillStyle = GREEN; ctx.beginPath(); ctx.arc(BASE_W / 2, ICO_Y, 25, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = WHITE; ctx.lineWidth = 4; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.beginPath(); ctx.moveTo(BASE_W/2 - 10, ICO_Y); ctx.lineTo(BASE_W/2 - 2, ICO_Y + 8); ctx.lineTo(BASE_W/2 + 12, ICO_Y - 9); ctx.stroke(); ctx.restore();

    ctx.fillStyle = DARK; ctx.font = 'bold 17px Arial'; ctx.textAlign = 'center'; ctx.fillText('Pembayaran Berhasil', BASE_W / 2, ICO_Y + 48); ctx.fillStyle = GREY; ctx.font = '12px Arial'; ctx.fillText(data.dateStr, BASE_W / 2, ICO_Y + 68);
    const NOM_Y = ICO_Y + 102; ctx.fillStyle = LGREY; ctx.fillRect(RX, NOM_Y - 30, RW, 60); ctx.fillStyle = GREY; ctx.font = '12px Arial'; ctx.fillText('Total Pembayaran', BASE_W / 2, NOM_Y - 6); ctx.fillStyle = DARK; ctx.font = 'bold 28px Arial'; ctx.fillText(formatRupiah(data.nominal), BASE_W / 2, NOM_Y + 24);

    ctx.strokeStyle = LINE; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(RX + 20, NOM_Y + 45); ctx.lineTo(RX + RW - 20, NOM_Y + 45); ctx.stroke();
    const QR_Y = NOM_Y + 110; drawQR(ctx, BASE_W / 2, QR_Y, 115); ctx.strokeStyle = '#dddddd'; ctx.lineWidth = 1; ctx.strokeRect(BASE_W/2 - 115/2 - 1, QR_Y - 115/2 - 1, 117, 117);
    ctx.fillStyle = '#333333'; ctx.font = 'bold 12px Arial'; ctx.fillText('QRIS', BASE_W / 2, QR_Y + 76); ctx.fillStyle = GREY; ctx.font = '10px Arial'; ctx.fillText(`NMID: ${data.merchant.nmid}`, BASE_W / 2, QR_Y + 92);

    ctx.beginPath(); ctx.moveTo(RX + 20, QR_Y + 108); ctx.lineTo(RX + RW - 20, QR_Y + 108); ctx.stroke();
    const DY = QR_Y + 132; ctx.fillStyle = DARK; ctx.font = 'bold 14px Arial'; ctx.textAlign = 'left'; ctx.fillText('Detail Transaksi', RX + 20, DY);
    ...
    renderRow('Nama Merchant', data.merchant.name, DY + 28); renderRow('Metode Bayar', data.payMethod, DY + 54); renderRow('No. Referensi', data.refNum, DY + 80); renderRow('No. RRN', data.rrn, DY + 106); renderRow('ID Transaksi', data.txId, DY + 132);

    const TEAR_Y = RY + RH - 55; const TOOTH = 10; const TEETH = Math.ceil(RW / TOOTH); ctx.fillStyle = BGOUT; ctx.beginPath(); ctx.moveTo(RX, TEAR_Y);
    for (let i = 0; i <= TEETH; i++) {
        const px = RX + i * TOOTH; const py = TEAR_Y - (i % 2 === 0 ? TOOTH : 0); ctx.lineTo(Math.min(px, RX + RW), py);
    }
...
function saveReceipt(canvas, uniqueId) {
    ...
    const filename = `receipt_${uniqueId}.png`; const fp = path.join(OUTPUT_DIR, filename); const buffer = canvas.toBuffer('image/png'); fs.writeFileSync(fp, buffer);
    return { fp, filename, buffer };
}

function generateAndSaveReceiptData() {
    const data = generateData(); const canvas = drawReceipt(data); const { fp, filename, buffer } = saveReceipt(canvas, data.fileUniqueTag);
...
async function uploadAndConfirmReceipt(cookie, filename, buffer, fp) {
    try {
        ...
        if (fs.existsSync(fp)) fs.unlinkSync(fp);
        const resData = error.response?.data; const httpStatus = error.response?.status;
...
async function runAccountLoop(account) {
    ...
    if (cookieAktif) {
        ...
        if (apaValid) {
            console.log(`   🚀 [VALID] Session cookie terverifikasi AKTIF!`);
            butuhLoginOtomatis = false;
        } else {
            console.log(`   ⚠️ [EXPIRED] Session cookie mati. Menyiapkan bypass 2captcha via API...`);
        }
    }

    // Eksekusi Login via HTTP POST API + 2Captcha Solver (Tidak membuka browser sama sekali)
    if (butuhLoginOtomatis) {
        console.log(`[${account.accountName}] Memproses login otomatis menggunakan token bypass 2captcha...`);
